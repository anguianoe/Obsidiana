...
public interface WorkspaceInvitationRepository extends JpaRepository<WorkspaceInvitation, UUID>, JpaSpecificationExecutor<WorkspaceInvitation> {
    ...
    List< WorkspaceInvitation > findAllByInvitedUserIdAndStatus( UUID userId, InvitationStatus status);

    Optional< WorkspaceInvitation > findByWorkspaceIdAndInvitedUser( UUID workspaceId, AppUser invitedUser);
